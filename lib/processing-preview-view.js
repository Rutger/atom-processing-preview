...
import * as modifySource from './modify-source';
...
class ProcessingPreviewView extends View {
	...
	renderSketch() {
		return this.getSketchSource()
			.then((code) => {
				const parentElement = this.canvas.context.parentElement;
				return modifySource.transcodeFullScreen(code, {
					width: parentElement.clientWidth,
					height: parentElement.clientHeight,
				});
			})
			.then(modifySource.transcodePixelDensity)
			.then((code) => this.renderSketchCode(code));
	}
	renderSketchCode(sketchCode) {
		if (!sketchCode) return;

		const sketchCompiled = Processing.compile(sketchCode).sourceCode;
